...
def test_attack_obj_fun(pokemons_list_fixture: list[Pokemon]) -> None:
    ant_list = [
        [9, 3, 4, 0, 6],
        [0, 1, -1, -1, -1],
        [5, 6, 4, 5, 3],
    ...
    ant = np.array(ant_list)
    expected_result = 194235.0
    assert attack_obj_fun(ant, pokemons_list_fixture) == expected_result


# Test case 1: Empty team
def test_generalist_team_fun_empty_team():
    ant = np.array([])
    pokemon_list = []
    result = generalist_team_fun(ant, pokemon_list)
    assert result == 0.0


# Test case 2: Team with none
def test_generalist_team_fun_none(pokemons_list_fixture: list[Pokemon]) -> None:
    pokemon_list = pokemons_list_fixture
    ant_list = [
        [0, 0, -1, -1, -1],
    ]
    ant = np.array(ant_list)
...
# Test case 3: Team with only spinner
def test_generalist_team_fun_spinner(pokemons_list_fixture: list[Pokemon]) -> None:
    pokemon_list = pokemons_list_fixture
    ant_list = [
        [0, 0, -1, -1, -1],
    ]
    ant = np.array(ant_list)
    with patch("poketactician.objectives.is_spinner", return_value=1.0):
        result = generalist_team_fun(ant, pokemon_list)
        assert result == 1.0


# # TODO Test case 4: Team with only hazard setter
# def test_generalist_team_fun_hazard_setter(pokemons_list_fixture: list[Pokemon]):
#     pokemon_list = pokemons_list_fixture
#     ant_list = [
#         [0, 0, -1, -1, -1],
#     ]
#     ant = np.array(ant_list)
#     with patch("poketactician.objectives.is_hazard_setter", return_value=1.0):
#         result = generalist_team_fun(ant, pokemon_list)
#         assert result == 1.0


# # TODO Test case 5: Team with only cleric
# def test_generalist_team_fun_cleric(pokemons_list_fixture: list[Pokemon]):
#     pokemon_list = pokemons_list_fixture
#     ant_list = [
#         [0, 0, -1, -1, -1],
#     ]
#     ant = np.array(ant_list)
#     with patch("poketactician.objectives.is_cleric", return_value=1.0):
#         result = generalist_team_fun(ant, pokemon_list)
#         assert result == 1.0


# # TODO Test case 6: Team with all roles
# def test_generalist_team_fun_all_roles(pokemons_list_fixture: list[Pokemon]):
#     pokemon_list = pokemons_list_fixture
#     ant_list = [
#         [0, 0, -1, -1, -1],
#     ]
#     ant = np.array(ant_list)
#     with patch("poketactician.objectives.is_hazard_setter", return_value=1.0), patch(
#         "poketactician.objectives.is_spinner", return_value=1.0
#     ), patch("poketactician.objectives.is_cleric", return_value=1.0):
#         result = generalist_team_fun(ant, pokemon_list)
#         assert result == 3.0


# Test case 1: Empty team
def test_defensive_team_fun_empty_team():
    ant = np.array([])
    pokemon_list = []
    result = defensive_team_fun(ant, pokemon_list)
    assert result == 0.0


# Test case 2: Team with none
def test_defensive_team_fun_none(pokemons_list_fixture: list[Pokemon]):
    pokemon_list = pokemons_list_fixture
    ant_list = [
        [0, 0, -1, -1, -1],
    ]
...
# # Test case 3: Team with only wall
# def test_defensive_team_fun_wall(pokemons_list_fixture: list[Pokemon]):
#     pokemon_list = pokemons_list_fixture
#     ant_list = [
#         [0, 0, -1, -1, -1],
#     ]
#     ant = np.array(ant_list)
#     with patch("poketactician.objectives.is_wall", return_value=1.0):
#         result = defensive_team_fun(ant, pokemon_list)
#         assert result == 1.0


# # Test case 4: Team with only phazer
# def test_defensive_team_fun_phazer(pokemons_list_fixture: list[Pokemon]):
#     pokemon_list = pokemons_list_fixture
#     ant_list = [
#         [0, 0, -1, -1, -1],
#     ]
#     ant = np.array(ant_list)
#     with patch("poketactician.objectives.is_phazer", return_value=1.0):
#         result = defensive_team_fun(ant, pokemon_list)
#         assert result == 1.0

# # Test case 5: Team with all
# def test_defensive_team_fun_all_roles(pokemons_list_fixture: list[Pokemon]):
#     pokemon_list = pokemons_list_fixture
#     ant_list = [
#         [0, 0, -1, -1, -1],
#     ]
#     ant = np.array(ant_list)
#     with patch("poketactician.objectives.is_wall", return_value=1.0), patch("poketactician.objectives.is_phazer", return_value=1.0):
#         result = defensive_team_fun(ant, pokemon_list)
#         assert result == 1.0


# Test case 1: Empty team
def test_offensive_team_fun_empty_team():
    ant = np.array([])
    pokemon_list = []
    result = offensive_team_fun(ant, pokemon_list)
    assert result == 0.0


# Test case 2: Team with none
def test_offensive_team_fun_none(pokemons_list_fixture: list[Pokemon]):
    pokemon_list = pokemons_list_fixture
    ant_list = [
        [0, 0, -1, -1, -1],
    ]
...
# Test case 3: Team with only special sweeper
def test_offensive_team_fun_special_sweeper(pokemons_list_fixture: list[Pokemon]):
    pokemon_list = pokemons_list_fixture
    ant_list = [
        [0, 0, -1, -1, -1],
    ]
    ant = np.array(ant_list)
    with patch("poketactician.objectives.is_special_sweeper", return_value=1.0):
        result = offensive_team_fun(ant, pokemon_list)
        assert result >= 1.0


# Test case 4: Team with only physical sweeper
def test_offensive_team_fun_physical_sweeper(pokemons_list_fixture: list[Pokemon]):
    pokemon_list = pokemons_list_fixture
    ant_list = [
        [0, 0, -1, -1, -1],
    ]
    ant = np.array(ant_list)
    with patch("poketactician.objectives.is_physical_sweeper", return_value=1.0):
        result = offensive_team_fun(ant, pokemon_list)
        assert result >= 1.0


# Test case 5: Team with one physical sweeper and one special sweeper
def test_offensive_team_fun_all(pokemons_list_fixture: list[Pokemon]):
    pokemon_list = pokemons_list_fixture
    ant_list = [
        [0, 0, -1, -1, -1],
    ]
    ant = np.array(ant_list)
    with patch("poketactician.objectives.is_special_sweeper", return_value=1.0), patch(
        "poketactician.objectives.is_physical_sweeper", return_value=1.0
    ):
        result = offensive_team_fun(ant, pokemon_list)
        assert result >= 2.0

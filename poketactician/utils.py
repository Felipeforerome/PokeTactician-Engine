...
def get_random_moves(lm: np.ndarray, i: int, random_state: np.random.Generator) -> np.ndarray:
    ...
    chosen = EMPTY_MOVE_SENTINEL * np.ones(NUMBER_OF_MOVES_SLOTS, dtype=np.int16)
    if len(legal_moves) >= NUMBER_OF_MOVES_SLOTS:
        selected = random_state.choice(legal_moves, size=NUMBER_OF_MOVES_SLOTS, replace=False)
    else:
...
class DecisionFunction(Protocol):
    def __call__(self, x: NDArray[np.int16], **kwargs) -> NDArray[np.int16]: ...
